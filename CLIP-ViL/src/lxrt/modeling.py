    ...
    def __init__(self, config):
        ...
        if self.visualbert_style:
            ...
            if config.side_config is not None:
                self.side_block_l, self.side_first_downsample_l, self.side_final_upsample_l, self.side_downsamples_l, self.side_gate_params_l = self.construct_side_network(
                    config, config.side_config, self.num_l_layers, BertLayer, 'l'
                )
        else:
            # Layers
            ...
            if config.side_config is not None:
                ...
                self.side_block_x, self.side_first_downsample_x, self.side_final_upsample_x, self.side_downsamples_x, self.side_gate_params_x = self.construct_side_network(
                    config, config.side_config, self.num_x_layers, LXRTXLayer, 'x'
                )

                self.side_block_r, self.side_first_downsample_r, self.side_final_upsample_r, self.side_downsamples_r, self.side_gate_params_r = self.construct_side_network(
                    config, config.side_config, self.num_r_layers, BertLayer, 'r'
                )
