...
from tasks.nlvr_data import NLVRDataset, NLVRTorchDataset, NLVREvaluator
...
def get_data_tuple(splits: str, bs:int, shuffle=False, drop_last=False, distributed=False, aspect_ratio_group_factor = -1, exhaustive = False) -> DataTuple:
    dset = NLVRDataset(splits)
    tset = NLVRTorchDataset(dset)
    evaluator = NLVREvaluator(dset)
...
class VQA(TrainerBase):
    ...
    def create_side_network_initial_weights(self):
        ...
        if "fisher" in self.args.load_side_pretrained_weights or "bert" in self.args.load_side_pretrained_weights:
            ...
            # prepare the dataset
            train_dataset = NLVRTorchDataset(NLVRDataset(self.args.train)) # ad-hoc writing
            ...
            if "fisher" in self.args.load_side_pretrained_weights:
                # ad-hoc writing
                importance_measure = compute_fisher(model, "nlvr", train_dataset, data_collator, num_samples=self.args.samples_for_fisher)
            else:
