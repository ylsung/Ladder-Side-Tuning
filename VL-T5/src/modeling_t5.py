...
from my_transformers.modeling_outputs import SideBaseModelOutputWithPastAndCrossAttentions, SideSeq2SeqLMOutput, SideBaseModelOutput
    ...
    def __init__(self, config, obj_order_embedding):
        ...
        if self.config.individual_vis_layer_norm:
            ...
            if self.config.use_vis_layer_norm:
                feat_embedding.append(T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, config=config))
            ...
            if self.config.use_vis_layer_norm:
                absolute_vis_pos_embedding.append(T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, config=config))
...
class JointEncoder(T5Stack):
    ...
    def forward(
        ...
        # inputs only containing text, this is used for backbone model 
        backbone_embeds = inputs_embeds
        ...
        backbone_extended_attention_mask = self.get_extended_attention_mask(backbone_attention_mask, (B, L), inputs_embeds.device)
        ...
        if not self.use_side_transformers:
            backbone_embeds = inputs_embeds
            backbone_attention_mask = attention_mask
            backbone_extended_attention_mask = extended_attention_mask
        ...
        if self.config.num_layers > 0:
            ...
            if not self.use_side_transformers:
                ...
                # [1, n_heads, Q_len, K_len]
                text_position_bias = self.side_block[self.first_available_layer].layer[0].SelfAttention.compute_bias(
                    L, L)
                num_heads = text_position_bias.size(1)
                side_position_bias = text_position_bias.new_zeros(
                    1, num_heads, seq_length, seq_length)
                side_position_bias[:, :, :L, :L] = text_position_bias
                ...
                # position_bias[:, :, L:, :] = 0
                # position_bias[:, :, :, L:] = 0
                side_position_bias = side_position_bias + extended_attention_mask
            ...
            for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
                ...
                layer_outputs = layer_module(
                    hidden_states,
                    attention_mask=backbone_extended_attention_mask,
                    ...
                    if side_layer_module is not None:
                        if not self.add_residual_after:
                            if self.use_gate == "learnable":
                                ...
                                hidden_states = side_downsample(layer_outputs[0])
                                dim = hidden_states.shape[-1]
                                device = hidden_states.device
                                hidden_states = torch.cat([hidden_states, torch.zeros((B, V_L, dim), device=device)], dim=1)
                                ...
                                device = hidden_states.device
                                hidden_states = torch.cat([hidden_states, torch.zeros((B, V_L, dim), device=device)], dim=1)
                                side_hidden_states = gate * side_hidden_states + (1 - gate) * hidden_states
                            else:
                                hidden_states = side_downsample(layer_outputs[0])
                                dim = hidden_states.shape[-1]
                                device = hidden_states.device
                                hidden_states = torch.cat([hidden_states, torch.zeros((B, V_L, dim), device=device)], dim=1)
                                side_hidden_states = side_hidden_states + hidden_states
    ...
    def forward(
        ...
        **kwargs,
        ...
        if vis_attention_mask is None:
            ...
            if self.use_side_transformers:
                V_L = side_hidden_states.size(1) - L # V + L combined inputs
            else:
                V_L = encoder_outputs[0].size(1) - L
            vis_attention_mask = attention_mask.new_ones(B, V_L)

        backbone_encoder_attention_mask = attention_mask # only for L
        
        encoder_attention_mask = torch.cat([attention_mask, vis_attention_mask], dim=1)

        if not self.use_side_transformers:
            # the backbone model uses text + vision input if not using side network
            backbone_encoder_attention_mask = encoder_attention_mask
        ...
        decoder_outputs = self.decoder(
            ...
            encoder_attention_mask=backbone_encoder_attention_mask,
            side_encoder_attention_mask=encoder_attention_mask,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs):
        ...
        if "side_past_key_values" in kwargs:
            output["side_past_key_values"] = kwargs["side_past_key_values"]
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs
        ...
        if is_encoder_decoder:
            ...
            if encoder_outputs.last_side_hidden_state is not None:
