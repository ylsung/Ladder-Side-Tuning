...
class T5LayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6, config=None):
        ...
        self.bitfit = config.adapter_config.bitfit if config.adapter_config is not None else False 
        if self.bitfit:
...
class T5LayerFF(nn.Module):
    def __init__(self, config):
        ...
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, config=config)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        if config.lora_config is not None:
            self.use_lora = True
            self.q = lora.LoRALinearController(self.d_model, self.inner_dim, config=config.lora_config, bias=False if not self.bitfit else True)
            self.v = lora.LoRALinearController(self.d_model, self.inner_dim, config=config.lora_config, bias=False if not self.bitfit else True)
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, task_embed=None):
        ...
        self.use_side_transformers = False
        if config.side_config is not None:
            side_config = config.side_config
            self.use_side_transformers = True
            config_for_side_block = copy.deepcopy(config)

            config_for_side_block.d_ff = config_for_side_block.d_ff // side_config.task_reduction_factor
            config_for_side_block.d_kv = config_for_side_block.d_kv // side_config.task_reduction_factor
            config_for_side_block.d_model = config_for_side_block.d_model // side_config.task_reduction_factor
            ...
            self.side_block = nn.ModuleList(
                [T5Block(config_for_side_block, has_relative_attention_bias=bool(i == self.first_available_layer)) 
            ...
            if side_config.task_reduction_factor == 1:
                self.side_first_downsample = nn.Identity()
            else:
                self.side_first_downsample = nn.Linear(config.d_model, config_for_side_block.d_model, bias=side_config.add_bias_sampling)

            if side_config.side_downsample_pool:
                self.side_downsamples = nn.ModuleList(
                    [nn.AdaptiveAvgPool1d(config_for_side_block.d_model)
                    if i in side_layers else None
                    for i in range(config.num_layers)]
                )
            else:
                if side_config.task_reduction_factor == 1:
                    ...
                    self.side_downsamples = nn.ModuleList(
                        [nn.Linear(config.d_model, config_for_side_block.d_model, bias=side_config.add_bias_sampling) 
                        if i in side_layers else None
                        for i in range(config.num_layers)]
                    )
            ...
            self.final_side_layer_norm = T5LayerNorm(config_for_side_block.d_model, eps=config_for_side_block.layer_norm_epsilon, config=config_for_side_block)

        self.final_layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon, config=config)
        self.dropout = nn.Dropout(config.dropout_rate)
    ...
    def forward(
        ...
        encoder_attention_mask=None,
        side_encoder_attention_mask=None,
        inputs_embeds=None,
        head_mask=None,
        ...
        if self.is_decoder and side_encoder_attention_mask is None and side_encoder_hidden_states is not None:
            encoder_seq_length = side_encoder_hidden_states.shape[1]
            side_encoder_attention_mask = torch.ones(
                batch_size, encoder_seq_length, device=inputs_embeds.device, dtype=torch.long
            )
        ...
        if side_past_key_values is None and self.use_side_transformers:
        ...
        if self.is_decoder and side_encoder_attention_mask is not None:
            side_encoder_extended_attention_mask = self.invert_attention_mask(side_encoder_attention_mask)
        else:
            side_encoder_extended_attention_mask = None
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.use_side_transformers = True if config.side_config is not None else False
        if self.use_side_transformers:
            if config.side_config.task_reduction_factor == 1:
                self.side_final_upsample = nn.Identity()
            else:
                self.side_final_upsample = nn.Linear(config.d_model // config.side_config.task_reduction_factor, config.d_model, bias=config.side_config.add_bias_sampling)
