...
class T5SideBlock(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            self_attn_past_key_value, cross_attn_past_key_value, side_cross_attn_past_key_value = None, None, None
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None, prefix_emb=None):
        ...
        self.side_gate_params = nn.ParameterList(
            [nn.Parameter(torch.ones(1) * 0.0) for i in range(config.num_layers)]
        )
