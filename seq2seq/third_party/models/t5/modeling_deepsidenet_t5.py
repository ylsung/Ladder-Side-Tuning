...
class T5LayerFF(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        self.alpha = config.deepnet_alpha if getattr(config, "use_deepnet_init", False) else 1
...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.alpha * hidden_states + self.dropout(y)
...
class T5LayerCrossAttention(nn.Module):
    ...
    def forward(
        ...
        layer_output = self.alpha * hidden_states + self.dropout(attention_output[0])
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            beta = getattr(module.config, "deepnet_beta", 1)
            print(module.config.is_decoder, beta)
            module.wi.weight.data.normal_(mean=0.0, std=factor * beta * ((self.config.d_model) ** -0.5))
            if hasattr(module.wi, "bias") and module.wi.bias is not None:
                module.wi.bias.data.zero_()
            module.wo.weight.data.normal_(mean=0.0, std=factor * beta * ((self.config.d_ff) ** -0.5))
            if hasattr(module.wo, "bias") and module.wo.bias is not None:
                module.wo.bias.data.zero_()
        elif isinstance(module, T5DenseGatedGeluDense):
            beta = getattr(module.config, "deepnet_beta", 1)
            print(module.config.is_decoder, beta)
            module.wi_0.weight.data.normal_(mean=0.0, std=factor * beta * ((self.config.d_model) ** -0.5))
            if hasattr(module.wi_0, "bias") and module.wi_0.bias is not None:
                module.wi_0.bias.data.zero_()
            module.wi_1.weight.data.normal_(mean=0.0, std=factor * beta * ((self.config.d_model) ** -0.5))
            ...
            module.v.weight.data.normal_(mean=0.0, std=factor * beta * (d_model ** -0.5))
            module.o.weight.data.normal_(mean=0.0, std=factor * beta * ((n_heads * key_value_proj_dim) ** -0.5))
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, adapter_config=None, prefix_emb=None):
        ...
        side_config.use_deepnet_init = True
        ...
        # side_upsamples = nn.Linear(side_config.d_model, config.d_model)
        self.side_upsamples = nn.ModuleList(
            [nn.Linear(side_config.d_model, config.d_model) for i in range(config.num_layers)]
        )
        
        # side_downsample = nn.Linear(config.d_model, side_config.d_model)
        ...
        if self.use_gate == "learnable":
            self.side_gate_params = nn.ParameterList(
                [nn.Parameter(torch.ones(1) * config.gate_alpha) for i in range(config.num_layers)]
            )

            self.gate_T = config.gate_T
        elif "schedule" in self.use_gate:
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if self.use_gate == "learnable":
                    ...
                    side_hidden_states = gate * side_hidden_states + (1 - gate) * layer_outputs[0] # add the information from the backbone network
                elif "schedule" in self.use_gate:
                    gate = getattr(self, f"side_gate_{i}")
                    side_hidden_states = gate * side_hidden_states + (1 - gate) * layer_outputs[0]
                else:
                    side_hidden_states = side_hidden_states + layer_outputs[0]
            ...
            # gate = torch.sigmoid(side_gate_param * 10)
            # side_hidden_states = gate * side_upsample(side_hidden_states) + (1 - gate) * hidden_states # add the information from the backbone network
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None):
        ...
        N = config.num_layers
        M = config.num_decoder_layers

        config.deepnet_alpha = 0.81 * (N ** 4 * M) ** (1 / 16)
        config.deepnet_beta = 0.87 * (N ** 4 * M) ** (- 1 / 16)
        ...
        decoder_config.deepnet_alpha = (3 * M) ** (1 / 4)
        decoder_config.deepnet_beta = (12 * M) ** (- 1 / 4)
