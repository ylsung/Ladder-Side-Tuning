...
from .modeling_side_outputs import SideBaseModelOutputWithPastAndCrossAttentions, SideSeq2SeqLMOutput, SideBaseModelOutput
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, side_embed_tokens=None, adapter_config=None, prefix_emb=None):
        super().__init__(config)

        self.embed_tokens = embed_tokens
        self.side_embed_tokens = side_embed_tokens
    ...
    def get_side_input_embeddings(self):
        return self.side_embed_tokens

    def set_side_input_embeddings(self, new_embeddings):
        self.side_embed_tokens = new_embeddings
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            side_inputs_embeds = self.side_embed_tokens(input_ids)
            ######################################
        ...
        side_hidden_states = self.dropout(side_inputs_embeds)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None, lora_config=None):
        ...
        self.shared = nn.Embedding(config.vocab_size, config.d_model)
        self.side_shared = nn.Embedding(config.vocab_size, config.d_model//adapter_config.task_reduction_factor)
        #############################################################
        self.prefix_tuning = config.prefix_tuning
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared, side_embed_tokens=self.side_shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared)
        ...
        self.decoder = T5Stack(decoder_config, self.shared, side_embed_tokens=self.side_shared, adapter_config=adapter_config, prefix_emb=self.prefix_shared)

        self.bitfit = adapter_config.bitfit if adapter_config is not None else False 
        self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=False if not self.bitfit else True)

        self.side_lm_head = nn.Linear(config.d_model//adapter_config.task_reduction_factor, config.vocab_size, bias=False if not self.bitfit else True)

        self.side_lm_gate = nn.Parameter(torch.ones(1) * config.gate_alpha)

        self.use_gate = config.use_gate
        if self.use_gate == "learnable":
            # Parameter list cannot contain None
            self.side_lm_gate_params = nn.ParameterList(
                [nn.Parameter(torch.ones(1) * config.gate_alpha) 
                for i in range(1)]
            )
            ...
            for i in range(1):
                self.register_buffer('side_lm_gate_' + str(i), torch.ones(1))
        elif self.use_gate == "none":
            pass
        else:
            raise NotImplementedError
    ...
    def get_side_input_embeddings(self):
        return self.side_shared

    def set_side_input_embeddings(self, new_embeddings):
        self.side_shared = new_embeddings
        self.encoder.set_side_input_embeddings(new_embeddings)
        self.decoder.set_side_input_embeddings(new_embeddings)

    def set_side_output_embeddings(self, new_embeddings):
        self.side_lm_head = new_embeddings

    def get_side_output_embeddings(self):
        return self.side_lm_head
    ...
    def forward(
        ...
        side_sequence_output = decoder_outputs.last_side_hidden_state
        ...
        if self.config.tie_word_embeddings:
            ...
            side_sequence_output = side_sequence_output * (self.model_dim ** -0.5)

        lm_logits = self.lm_head(sequence_output)
        side_lm_logits = self.side_lm_head(side_sequence_output)

        # print(lm_logits.shape, side_lm_logits.shape)

        if self.use_gate == "learnable":
            side_gate_param = self.side_lm_gate_params[0]
            gate = torch.sigmoid(side_gate_param / self.gate_T)
            lm_logits = gate * side_lm_logits + (1 - gate) * lm_logits # add the information from the backbone network
        elif "schedule" in self.use_gate or self.use_gate == "one":
            gate = getattr(self, f"side_lm_gate_0")
            lm_logits = gate * side_lm_logits + (1 - gate) * lm_logits
        else:
            lm_logits = side_lm_logits + lm_logits
...
if __name__ == "__main__":
    ...
    t = AutoTokenizer("t5-base")
    m = T5ForConditionalGeneration.from_pretrained("t5-base")

    inputs = t("I like to eat", return_tensors="pt", return_dict=True)
